...
A description of the `tidyjson` package on CRAN 
...
Using `tidyjson` we can extract key information from the JSON structure using the pipeline operator *%>%*.  In this case, the JSON structure is complex, we'll use the *enter_object()* function to move into a specific object key in the JSON attribute.
...
*(**Note** - some of the values such as abstract and keywords are not being displayed due to the length of the text.)*
...
## Use Case - Graph the 100 Most Viewed New York Times Articles by Section 

For this particular example, use the NY Times Most Popular API to retrieve the 100 most viewed articles for a single day.  This will require using the offset parameter in the API, ranging from 20, 40, ... 100.  
...
for (i in 1:5) {
    ...
    results <-
     ...
     if (i == 1) { 
          top_100_json <- results
     } 
     else {
          top_100_json <- rbindlist(list(top_100_json, results))
     }
